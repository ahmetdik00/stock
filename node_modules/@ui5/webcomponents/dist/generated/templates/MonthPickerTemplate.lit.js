...
const block2 = (item, index, context, tags, suffix) => html`<div data-sap-timestamp=${ifDefined(item.timestamp)} tabindex=${ifDefined(item._tabIndex)} ?data-sap-focus-ref="${item.focusRef}" class="${ifDefined(item.classes)}" role="gridcell" aria-selected="${ifDefined(item.ariaSelected)}"><span class="ui5-dp-monthtext">${ifDefined(item.name)}</span>${ item.nameInSecType ? block3(item, index, context, tags, suffix) : undefined }</div>`;
const block3 = (item, index, context, tags, suffix) => html`<span class="ui5-dp-monthtext ui5-dp-monthsectext">${ifDefined(item.nameInSecType)}</span>`;
